...
delete VN_EMP_LATE where YEAR='<year/>' and MONTH='<Month/>' and ISLATE='0' and EMPLOYEENO='<EmpID/>'
...
SELECT Distinct E.EmpID,E.CBName,E.DeptCode,'<year/>','<Month/>','0'
from   PWERP_MS.dbo.RsKQResult K,   PWERP_MS.dbo.RsEmployee E ,  PWERP_MS.dbo.RsBasTurn B
	Where  K.EmpCode=E.EmpCode
	AND K.Turn= B.Code
	AND E.Empid='<EmpID/>'
...
while @@fetch_status=0      --返回被 FETCH 语句执行的最后游标的状态
 begin
				  ...
				  Set @SQL=' update VN_EMP_LATE set  WT'+@DAYCOUNT+'='''+@WORKTIME+''' where EMPLOYEENO='+@inputEmpID+' and YEAR=''<year/>'' and MONTH=''<Month/>'' and ISLATE=''0'' '
				   Exec (@SQL)
				    Insert Into #SAL(SSQL ,lateCount,lateH,lateM)VALUES(@SQL,'','','')
...
IF('<year/>'=(SELECT DATENAME (YEAR ,getdate())))
	   begin
	    IF('<Month/>'<(SELECT DATENAME (MONTH ,getdate())))
			begin
				INSERT INTO VN_INEMP_LATE([YEAR],[MONTH] ,[ISLATE],[EMPLOYEENO])VALUES ('<year/>','<Month/>','0','<EmpID/>')
			END
	END
