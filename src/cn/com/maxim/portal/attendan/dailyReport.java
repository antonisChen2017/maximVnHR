 ...
 *
 */
public class dailyReport extends TemplatePortalPen
{
	...
	private void showHtml(Connection con, PrintWriter out, leaveCardVO lcVo , UserDescriptor UserInformation) throws SQLException {
			...
			String htmlPart1=hu.gethtml(UrlUtil.dailyReport);
			...
			htmlPart1=htmlPart1.replace("<SearchEmployeeNo/>",ControlUtil.drawSelectDBControl(con, out, "searchEmployeeNo", "VN_EMPLOYEE", "EMPLOYEENO", "EMPLOYEENO", " DEPARTMENT_ID='" + lcVo.getSearchDepartmen()+"'", lcVo.getSearchDepartmen()));
			htmlPart1=htmlPart1.replace("<SearchEmployee/>",ControlUtil.drawSelectDBControl(con, out, "searchEmployee", "VN_EMPLOYEE", "ID", "EMPLOYEE", " DEPARTMENT_ID='" + lcVo.getSearchDepartmen()+"'", lcVo.getSearchDepartmen()));

			if(lcVo.isShowDataTable()){
				htmlPart1=htmlPart1.replace("<drawTableM/>",HtmlUtil.drawLeaveCardTable(
						SqlUtil.getdailyReport(lcVo),HtmlUtil.drawTableMcheckButton(),  con, out,UrlUtil.pageMsList));
			}
