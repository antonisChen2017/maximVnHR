...
public class ad_editRole extends TemplatePortalPen
{
	Log4jUtil lu = new Log4jUtil();
	Logger logger = lu.initLog4j(ad_editRole.class);
	@Override
		...
		try
		{
				...
				{		
					...
					if (actText.equals("update")) {
						logger.info("設定員工角色/update: " +lcVo.toString());	
						lcVo.setShowDataTable(true);
	...
	private void showHtml(Connection con, PrintWriter out, leaveCardVO lcVo , UserDescriptor UserInformation) throws Exception {
			...
			HtmlUtil hu=new HtmlUtil();
			String htmlPart1=hu.gethtml(htmlConsts.html_ad_editRole);
