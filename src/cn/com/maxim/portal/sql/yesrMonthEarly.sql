...
delete VN_YEAR_MONTH_LATE where YEAR='<year/>' and MONTH='<Month/>' and ISLATE='0'
...
--定义游标
declare @SQL Varchar(4000), @lateETimeValue Varchar(10)
...
declare Select_late cursor for
        ...
        ,DAY16,DAY17,DAY18,DAY19,DAY20,DAY21,DAY22,DAY23,DAY24,DAY25,DAY26,DAY27,DAY28,DAY29,DAY30,DAY31 from VN_YEAR_MONTH_LATE
         WHERE YEAR='<year/>' and MONTH='<Month/>'  AND ISLATE='0'
   ...
   while @@fetch_status=0      --返回被 FETCH 语句执行的最后游标的状态
	  begin
	   ...
	   if(@DAY31 is not null and cast(@DAY31 as float)>0)
	    ...
	    Set @SQL=' update VN_YEAR_MONTH_LATE set LATETIMES='+convert(varchar(20),@lateCount)+' , HOUR='+cast(@lateH as nvarchar(10))+' ,MINUTE='+cast(@lateM as nvarchar(10))+' where EMPLOYEENO='''+@EMPLOYEENO+''' and YEAR='+@YEAR+' and MONTH='+@MONTH+' and ISLATE=0'
 	...
 	IF('<year/>'=(SELECT DATENAME (YEAR ,getdate())))
	   begin
	    IF('<Month/>'<(SELECT DATENAME (MONTH ,getdate())))
			begin
				INSERT INTO [hr].[dbo].[VN_ISQUERY_LATE]([YEAR],[MONTH] ,[ISLATE])VALUES ('<year/>','<Month/>','0')
			END
	END
