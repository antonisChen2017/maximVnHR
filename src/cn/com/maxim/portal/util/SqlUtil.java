...
public class SqlUtil
{
	...
	public static final String getrev_ExceedOvertime(overTimeVO otVo)
	{
		...
		StringBuilder Sb = new StringBuilder(" select  S.ID  \n")
			...
			//    Sb.append("	AND  S.STATUS='UB'   \n");
				//时间區間
			//	if (!otVo.getStartQueryDate().trim().equals(otVo.getEndQueryDate().trim()))
		//		{
		//			Sb.append(" AND   S.SUBMITDATE BETWEEN '" + otVo.getStartQueryDate() + " 00:00:00" + "'  AND '" + otVo.getEndQueryDate() + " 23:59:59' \n");
			//	}
	...
	//	Sb.append("	AND S.MONTHOVERTIME='"+otVo.getMonthOverTime()+"'  \n");
		...
		//	Sb.append(" AND (EP.ROLE='M' AND S.STATUS='T'  OR EP.ROLE='E' AND S.STATUS='UT' OR EP.ROLE='E' AND S.STATUS='UD' )  \n");
	...
	public static final String setExceedOvertimeS()
	{
		...
		StringBuilder Sb = new StringBuilder(" INSERT INTO VN_OVERTIME_S	 ")
				...
				.append(" ,NOTE ,STATUS ")
				.append(" ,USERREASONS ") 
				.append(" ,submitDate,APPLICATION_HOURS ") 
				.append(" ,OVERTIME_START,OVERTIME_END ") 
				.append(" ,TURN,SAVETIME) ")
				.append(" VALUES(?,? ")
				.append(",?,? ")
				.append(",?,? ")
				.append(",?")
	...
	{
		StringBuilder Sb = new StringBuilder(" declare @Res nvarchar(10)       "
				).append(" select @Res =   ")
				.append("  case  when ( (  SELECT  REASONS FROM VN_OVERTIME_S WHERE ID=" + ID + ") =  '5'    ")
				.append("		)  then (SELECT  USERREASONS FROM VN_OVERTIME_S WHERE ID=" + ID + ")     ")
				.append("	else (SELECT RS.REASONS FROM VN_OVERTIME_S as S INNER JOIN VN_REASONS AS RS ON S.REASONS = RS.ID WHERE S.ID=" + ID + " ) end  select @Res as flag    ");
		return Sb.toString();
	}
	...
	public static final String saveSalesCard()
	{
		...
		StringBuilder Sb = new StringBuilder(" update VN_LEAVECARD	 ")
				.append(" (EP_ID, HD_ID, ")
				.append("  APPLICATIONDATE,STARTSALESDATE, ")
				.append(" ENDSALESDATE,AGENT, ")
				.append(" NOTE,SALES_STATUS,")
				.append(" SALES_SAVETIME) ")
				.append("  VALUES(?,?,?,?,?,?,?,?,getdate()) ");
	...
	public static final String getSalesLeaveCard(leaveCardVO lcVo)
	{
		...
		StringBuilder Sb = new StringBuilder(" SELECT  C.ID,  ")
				...
				.append("  CONVERT(varchar(100), C.STARTLEAVEDATE, 120)  as '請假開始时间', ")
				.append("  CONVERT(varchar(100), C.ENDLEAVEDATE, 120)   as '請假結束时间', ")
				.append("  CONVERT(varchar(100), C.STARTSALESDATE, 120)  as '銷假開始时间', ")
				.append("  CONVERT(varchar(100), C.ENDSALESDATE, 120)   as '銷假結束时间', ")
				.append("  EP2.EMPLOYEE  as '代理人', ")
				.append("  C.NOTE as '備註', ")
				.append("  C.SALES_STATUS   as 'action' ,  ")
				.append("  C.RETURNMSG  as 'returnMSG' ")
				.append("  FROM VN_LEAVECARD  AS C  ")
				.append("  INNER JOIN      ")
				.append("  HR_EMPLOYEE AS EP      ON C.EP_ID =EP.ID  ")
	...
	public static final String getEmployeeID(String ID)
	{
		...
		StringBuilder Sb = new StringBuilder(" SELECT ")
				.append(" E.ID ,U.ID as UID,ENTRYDATE,DUTIES,U.UNIT,D.DEPARTMENT,E.EMPLOYEENO,E.DEPARTMENT_ID as DID, E.EMPLOYEE,E.ROLE ")
				.append(" FROM HR_EMPLOYEE as E ")
				.append("	INNER JOIN ")
				.append("		VN_DEPARTMENT AS D  ")
				.append("		ON D.ID = E.DEPARTMENT_ID ")
	...
	public static final String saveStopWorking()
	{
		...
		StringBuilder Sb = new StringBuilder(" INSERT INTO VN_STOPWORKING	 ")
				.append(" (EP_ID,DAYCOUNT, ")
				.append(" NOTE,STARTSTOPDATE, ")
				.append(" ENDENDDATE,REASON_ID, ")
				.append(" CREATEDAY) ")
				.append("  VALUES(?,?,?,?,?,?,getdate()) ");
	...
	public static final String getMonthReport(Connection con,repAttendanceVO raVo)
	{
		...
		for(int i=0;i<listDates.size();i++){
			lcVo.setApplicationDate(String.valueOf(listDates.get(i)));
			getMonthdailyReport(con,lcVo);
	 ...
	 * @return
	 */
	public static final String getMonthdailyReport(Connection con,leaveCardVO lcVo)
	{
		Log4jUtil lu=new Log4jUtil();
		Logger logger  =lu.initLog4j(SqlUtil.class);
		HtmlUtil hu=new HtmlUtil();
	...
	public static final String getlateOutEarly(Connection con,lateOutEarlyVO erVo)
	{
		...
		/* sql=hu.gethtml(sqlConsts.sql_lateIsQuery);
			...
			sql=sql.replace("<Month/>", erVo.getQueryYearMonth().split("/")[1]);
			sql=sql.replace("<isLate/>",IsLate);
			logger.info("getlateOutEarly  sql="+ sql);
			String LATE=DBUtil.queryDBField(con,sql,"LATE");
		if(Integer.valueOf(LATE)==0){*/
			if(erVo.getQueryIsLate().equals("1")){
			...
			if(erVo.getQueryIsLate().equals("2")){//早退名單
	...
	//	System.out.println("edVo.getHolidIyName() sql="+edVo.getHolidIyName());
		sql=sql.replace("<HOLIDAYNAME/>",edVo.getHolidIyName());
	//	System.out.println("edVo.gettHolidIyClas() sql="+edVo.getHolidIyClas());
	...
	public static final String  saveStopReasons(editStopReasonVO edVo) throws Exception
	{
		...
		//System.out.println("edVo.getHolidIyName() sql="+edVo.getStopReason());
